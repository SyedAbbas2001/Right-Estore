    ...
    <Link href={`/products/${product.slug}`} className="block group">
      <motion.div whileHover={{ y: -6 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-50"
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2">
            <motion.button onClick={handleWish}
              ...
              className={`w-9 h-9 rounded-full shadow-lg flex items-center justify-center transition-colors ${wished ? 'bg-pink-500 text-white' : 'bg-white text-gray-400 hover:text-pink-500'}`}>
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {product.badge && (
              <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}
                className={`badge ${product.badgeColor} text-[10px] shadow-md`}>
                {product.badge}
              </motion.span>
            )}
            {product.discount > 0 && (
          ...
          <div className="absolute bottom-0 left-0 right-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
            <motion.button onClick={handleCart} whileTap={{ scale: 0.97 }}
              className={`w-full py-3 font-bold text-sm flex items-center justify-center gap-2 transition-colors ${
                added ? 'bg-emerald-500 text-white' : 'bg-gradient-to-r from-pink-500 to-purple-600 text-white hover:from-purple-600 hover:to-pink-500'
              }`}>
        ...
        <div className="p-3 sm:p-4">
          <p className="text-[10px] sm:text-xs font-black text-purple-500 uppercase tracking-widest mb-1 capitalize">{product.category}</p>
          ...
          <div className="flex items-center gap-1 mb-2">
            ...
            <span className="text-[10px] text-gray-400 font-semibold">({product.reviews})</span>
          </div>
          ...
          {product.sizes?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {product.sizes.slice(0, 3).map(s => (
                <span key={s} className="text-[9px] font-black text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-md">{s}</span>
              ))}
              {product.sizes.length > 3 && <span className="text-[9px] font-black text-purple-400">+{product.sizes.length - 3}</span>}
            </div>
          )}
        </div>
      </motion.div>
